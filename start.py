...
from constants import confidence_set_methods, data_generation_functions
from models import fit_dml_model
from utils import calculate_range_length
...
class Simulator:
    ...
    def run_single_simulation(self, data, confidence_set_method, dml_model):
        ...
        """
        try:
            confidence_set = confidence_set_methods[confidence_set_method].get_confidence_set(data=data, dml_model=dml_model)
            true_ATE = data["true_ATE"]
            coverage_indicator = any(true_ATE >= start and true_ATE <= end for start, end in confidence_set)
    ...
    def run_simulations(self, n_samples_list, n_simulations, confidence_set_methods, data_generation_name="linear"):
        """
        ...
        for instrument_decay in [False, True]:
            ...
            instrument_type = "weak instrument" if instrument_decay else "strong instrument"
            ...
            for n in n_samples_list:
                for _ in tqdm(range(n_simulations), desc=f"Running simulations for n={n}"):
                    if instrument_decay:
                        ...
                        data = data_generation_functions[data_generation_name](n_samples=n)
            ...
            self._analyze_and_save_results(output, data_generation_name, instrument_decay, title_coverage, title_length)

    def _analyze_and_save_results(self, output, data_generation_name, instrument_decay, title_coverage, title_length):
        """
        ...
        coverage_summary = output_df.groupby(["n_samples", "method"])["coverage"].mean().reset_index()
        length_summary = output_df.groupby(["n_samples", "method"])["length"].median().reset_index()
        ...
        coverage_summary.to_csv(f"{self.output_dir}/coverage_summary_{file_prefix}.csv", index=False)
        length_summary.to_csv(f"{self.output_dir}/length_summary_{file_prefix}.csv", index=False)
        
        # Create and save plots
        self._create_coverage_plot(coverage_summary, instrument_decay, title_coverage, file_prefix)
        self._create_length_plot(length_summary, title_length, file_prefix)

    def _create_coverage_plot(self, coverage_summary, instrument_decay, title, file_prefix):
        ...
        for method in coverage_summary["method"].unique():
            ...
            plt.plot(method_data["n_samples"], method_data["coverage"], label=method, marker='o')
    ...
    def _create_length_plot(self, length_summary, title, file_prefix):
        ...
        for method in length_summary["method"].unique():
            ...
            plt.plot(method_data["n_samples"], method_data["length"], label=method, marker='o')
...
def parse_args():
    ...
    parser.add_argument("--n_samples", type=int, nargs='+', default=[150], help="List of sample sizes")
    parser.add_argument("--n_simulations", type=int, default=5, help="Number of simulations to run")
    parser.add_argument("--confidence_set_methods", type=str, nargs='+', default=["DRML", "Score"], help="List of confidence set methods")
